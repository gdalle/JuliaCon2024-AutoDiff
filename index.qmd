...
- **Manual** (aka blood, sweat and tears): work out $f'$ by hand
- **Numeric** (aka finite differences): $f'(x) \approx \frac{f(x+\varepsilon) - f(x)}{\varepsilon}$
- **Symbolic** (aka computer algebra): code a formula for $f$, get a formula for $f'$
- **Algorithmic** (aka autodiff): code a program for $f$, get a value for $f'(x)$
...
$$
v^\top J v = (((v^\top J_L) J_{L-1}) \dots J_2)J_1
$$
...
![Sparse autodiff with coloring [@gebremedhinWhatColorYour2005]](img/coloring.png)
